...
> D-18（2026-09-25）：用户指出安装包约175 MB、当前功能少，要求缩小包体。先按正式APK分析代码、原生库与资源占用，再裁剪当前产品未使用的内容；不得以删除已有收发、历史、账号、项目或必要动画功能换取体积数字。交付记录实际APK大小与升级验收，不预先承诺目标大小。

> D-17（2026-09-25）：用户要求手机对话保存在本地，进入时先显示已缓存内容，联网后再刷新，避免每次进入都重新加载。沿用已批准的按账号分区、有容量上限的正文缓存设计；刷新补充变化并保留阅读位置。缓存正文不作为最新运行状态或发送授权，离线草稿保留，恢复网络不自动重发指令。

> D-16（2026-09-25）：用户查看对话页两版图后选择“方案 B：状态放顶部，底部只留输入框”。状态、异常与待处理入口整合到固定顶栏，详细说明按需打开；不因状态文字变化推动正文和输入框，保留轻动画、真实审批及未知状态。图及规格见[顶栏方案 B](../design/mobile-ui-2026-09-25/chat-status-header.md)。

> D-15（2026-09-25）：用户明确将管理员预建账号最低密码长度由12位改为8位，并使用其提供的密码创建独立账号；不覆盖旧账号或迁移其内容。密码不写入文档、源码或公开诊断。

> D-14（2026-09-25）：用户明确“电脑”详情里的查看会话与项目入口应进入所属电脑的子页面，不切回全局“会话”页。导航为电脑详情→电脑全部会话或所选项目会话→具体对话，返回逐层回原电脑／项目并保留阅读位置；首页最近列表与其筛选不被这些操作改变。复用现有会话、来源与分页，保留原生转场动画，不增加平行数据源。已进入Android1.0.27，两个项目的实际导航、返回及标题验证见[本批验收](../validation/mobile-state-recovery.md)。

> D-13（2026-09-25）：用户明确允许手机点开尚未在原生桌面加载的已有会话时，自动让电脑Codex切到同一会话。沿用官方既有任务入口，不新建、不接管执行；正常已加载会话不跳转，后台发现／控制刷新／续租不得反复切换桌面。当前原生接口未提供不切页的后台加载能力，页面切换是已告知并获准的行为。已实现并装入Mac build47，真实冷会话打开、收发验收见本批状态记录。

> D-12（2026-09-25）：用户要求“电脑”入口可查看对应电脑的全部项目，再查看项目内的对话；“会话”入口作为有数量上限的最近对话列表，新对话自动加入并置前。用户确认“默认50，可在我的设置”：默认最多50条，可在“我的”中调整数量。超出上限仅移出最近列表，完整历史仍可从电脑／项目进入。沿用D-10单列表，不恢复分类或另建消息／状态来源。此条已实现并通过源码与专项验证，最终APK实际验收见本批状态记录。

> D-11（2026-09-25）：用户要求 Mac 设置采用“左边为选择，右边是详细信息”，避免所有设置挤在同一长页。沿用当前原生窗口，左侧固定中文分类，右侧显示并独立滚动当前内容，保留轻量切换动画；账号与本机监控状态继续复用原有入口。此项已明确要求实施，不需要重新选图。
...
> 阶段顺序确认（2026-09-24）：用户要求“功能实现完成后，告诉我，然后我们开始整理界面”。先完成已确认功能及实际联调，主管明确汇报已通过项与剩余限制后，再与用户进入界面整理。当前不启动 D-10 视觉改版，也不以构建、安装或模拟器局部通过宣称全部功能完成。
