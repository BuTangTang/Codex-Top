...
应用图标源文件为 `Resources/AppIcon.png`，构建脚本使用系统 `sips` 和 `iconutil` 生成完整的 16–1024 px ICNS，并写入 `CFBundleIconFile`。原图与[设计说明](design/logo-v1.md)纳入源码；中间 iconset 和 ICNS 留在构建目录。当前默认版本 `1.0.0`、构建号 `12`，内置屏与外接屏统一采用原外接屏紧凑字号，保留圆环拖动修复及“＋ / ⋯”标题栏；本轮不覆盖既有 Release 附件；图标安装验收仍对应历史 build 2。
