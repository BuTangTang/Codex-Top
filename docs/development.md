...
应用图标源文件为 `Resources/AppIcon.png`，构建脚本使用系统 `sips` 和 `iconutil` 生成完整的 16–1024 px ICNS，并写入 `CFBundleIconFile`。原图与[设计说明](design/logo-v1.md)纳入源码；中间 iconset 和 ICNS 留在构建目录。当前默认版本 `1.0.0`、构建号 `15`，跨屏拖动交给系统窗口服务，内置屏与外接屏采用统一紧凑字号，保留“＋ / ⋯”标题栏；本轮不覆盖既有 Release 附件；图标安装验收仍对应历史 build 2。
...
可设置 `VERSION=1.0.0`、`CONFIGURATION=release`。`SIGN_IDENTITY` 可指定本机已有签名身份；默认 `-` 是 ad-hoc 签名，不能等同 Developer ID 分发或 Apple 公证。签名身份、证书和本机设置不得提交。脚本不发布 GitHub Release，也不申请证书或公证。
...
```sh
swift test
VERSION=1.0.0 bash scripts/build-app.sh --universal
