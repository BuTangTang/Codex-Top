...
- [build 35 可选渐隐双弧](validation/twin-arc-orb.md)：保留经典圆环，新增 D4 双弧及固定运行数，移除机器人原型。
...
当前源码与本机安装为 v1.0.0 build 35。D-80 保留经典圆环、新增渐隐双弧并移除机器人原型；既有窗口布局、开合动画和任务逻辑保持。构建与当前实际检查范围见 [本轮验收](validation/twin-arc-orb.md)；历史完整连续动画与跨屏验收不借用本轮结论。中英文 README 四模式图片仍对应 build 10。公开 Release 附件未变。
