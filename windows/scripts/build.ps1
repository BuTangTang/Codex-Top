...
try {
    ...
    if ($Publish) {
        ...
        Compress-Archive -Path (Join-Path $projectRoot 'artifacts/CodexTop-Windows-x64') -DestinationPath (Join-Path $projectRoot 'artifacts/CodexTop-Windows-x64-0.1.11.zip') -Force
        & (Join-Path $PSScriptRoot 'package-source.ps1')
        Get-FileHash -LiteralPath (Join-Path $projectRoot 'artifacts/CodexTop-Windows-x64-0.1.11.zip') -Algorithm SHA256
    }
