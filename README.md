...
当前源码与本机安装为 **v1.0.0 build 35**。圆环外观可选“经典圆环／渐隐双弧”，保留原圆环，移除机器人原型。新双弧保持 44pt 和固定运行数，黑白主题使用更清楚的渐隐细弧；窗口开合、任务列表与四种显示方式保持。构建和实机检查见[本轮验证及边界](docs/validation/twin-arc-orb.md)。双列快捷菜单、7 天自动整理、右键整理和 80%–120% 比例保留。[已发布安装包](https://github.com/BuTangTang/Codex-Top/releases)与源码版本分别记录。
