...
当前源码与本机构建为 **v1.0.0 build 10**。四种模式的面板右上角统一为“＋ / ⋯”：加号选择任务，更多菜单管理显示方式、主题和设置。按钮与文字恢复初版尺寸。详见[本轮实机验收](docs/validation/appearance-restore.md)；[已发布安装包](https://github.com/BuTangTang/Codex-Top/releases)与源码版本分别记录。
...
圆环收起后仍为 44pt。浮窗使用紧凑单行列表，其余展开模式显示任务说明；四者共用关注列表和右上角按钮。[截图来源与验证范围](docs/validation/appearance-restore.md)。
...
- **外观与位置**：在“⋯ → 主题”切换黑色/浅色玻璃，保留扩散过渡与对应的窗口轮廓。显示比例为 60%–120%，每 5% 一档，可恢复 100%；圆环保持 44pt。外接屏和常驻浮窗采用紧凑字号。标题、任务名与辅助文字恢复原先字号和字重，小比例保留可读下限。
...
应用激活时，`⌘,` 打开设置，`⌘T` 显示任务。应用或监控窗口取得键盘焦点时，`⌘+`／`⌘=` 放大、`⌘-` 缩小，每次 5%，范围仍为 60%–120%。这些是应用内快捷键，不全局拦截其他应用的按键。设置跟随打开时鼠标所在屏幕，已在同一屏幕内显示时保留原位置。详细操作见[使用说明](docs/usage.md)。
