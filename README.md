...
当前源码与本机安装为 **v1.0.0 build 30**。本轮只调整更多菜单：四种模式两列排列、主题横排，固定 208pt 宽、12pt 字号，优先显示在监控右侧。159 项测试通过，真实包已核对菜单外观、完整入口、Esc 关闭和键盘进入设置，见[本轮验证及边界](docs/validation/quick-menu-grid.md)。既有列表、动画、7 天自动整理、跟随系统主题、右键整理和 80%–120% 比例保留。[已发布安装包](https://github.com/BuTangTang/Codex-Top/releases)与源码版本分别记录。
