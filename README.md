...
当前源码与本机构建为 **v1.0.0 build 9**。四种模式的面板右上角统一为“＋ / ⋯”：加号选择任务，更多菜单管理显示方式、主题和设置。文字采用清晰的主辅两级字号。详见[本轮实机验收](docs/validation/header-actions.md)；[已发布安装包](https://github.com/BuTangTang/Codex-Top/releases)与源码版本分别记录。
...
以下四张为 **v1.0.0 build 9 在 Mac 内置 Retina 屏上的实际窗口截图**，显示比例 100%。拍摄使用正式应用的隔离演示模式和合成任务，未读取真实任务或账户额度；不是设计图。图片为原始窗口捕获，未放大或重绘。

| 刘海模式 | 常驻浮窗 |
|---|---|
| ![内置屏刘海面板](docs/images/1.0.0-build9/notch-dark.jpg) | ![内置屏常驻浮窗](docs/images/1.0.0-build9/floating-dark.jpg) |

| 圆环展开 | 仅状态栏展开 |
|---|---|
| ![内置屏圆环展开面板](docs/images/1.0.0-build9/orb-expanded-dark.jpg) | ![内置屏状态栏展开面板](docs/images/1.0.0-build9/menubar-dark.jpg) |

圆环收起后仍为 44pt。浮窗使用紧凑单行列表，其余展开模式显示任务说明；四者共用关注列表和右上角按钮。[截图来源与验证范围](docs/validation/header-actions.md)。
...
- **外观与位置**：在“⋯ → 主题”切换黑色/浅色玻璃，保留扩散过渡与对应的窗口轮廓。显示比例为 60%–120%，每 5% 一档，可恢复 100%；圆环保持 44pt。外接屏和常驻浮窗采用紧凑字号。面板标题和任务名使用主字号，说明、计时、数量和额度小 2pt；小比例保留可读下限。
- **自由拖动**：常驻浮窗与展开圆环的顶部除按钮外均可拖动。拖到屏幕顶部不会自动吸附或切状态栏，显示方式由菜单切换；展开圆环移动后收回到新的球位置。
- **两个操作入口**：右上角始终显示“＋ / ⋯”，两者采用一致的可见宽度和点击区域。“＋”选择任务；“⋯”提供显示方式、主题、设置和收起。选择常驻浮窗后隐藏其他监控窗口，关闭浮窗返回进入前的模式；系统状态栏入口保留。
